...
import { createStyle } from "./CategoriesDropdown.style";
...
function CategoriesDropdown(props: { setOpenCategoriesDropdown: any }) {
    ...
    const CategoriesPage = (element: any) => {
        router.push(`/categories?searchProduct=${element}`)
        props.setOpenCategoriesDropdown()
    }
                    ...
                    <Box className={classes.optionsBox}>
                        {
                            apparelsData.filter(x => x.id === value).map((item, id) => (
                                <Box mt={20} key={id}>
                                    {item.name.map((element, index) => (
                                        <Flex key={index} className={classes.options}
                                            onClick={() => CategoriesPage(element)}>
                                            <BaseText className={classes.optionsText} style={typography.buttonText[i18nStore.getCurrentLanguage()].b3} >{element}</BaseText>
